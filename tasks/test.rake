...
task :default do
  ...
  unless ENV['start_from']
    ...
    [
      ...
      pages = count / 20
      puts "%3d #{collection_name} results pages" % (pages + 1)
      pages.times do |n|
  ...
  [
    ...
    query.each_with_index do |object,index|
      start ||= object['_id'] == ENV['start_from']
      if start
        suffixes = ['']
